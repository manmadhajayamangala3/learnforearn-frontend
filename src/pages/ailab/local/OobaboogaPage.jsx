    ...
    <div style={{ minHeight: '100vh', background: bg, color: txt, fontFamily: "'Rajdhani', sans-serif", overflowX: 'hidden' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: 860, margin: '0 auto', padding: '2rem 1.25rem 5rem' }}>
        ...
        <Block dark={dark} border={border} card={card}>
          ...
          <InfoBox color={color} dark={dark}>text-generation-webui is a Gradio-based Python web application that runs locally and provides a browser interface for loading and chatting with LLMs. It supports multiple backends (llama.cpp, ExLlamaV2, Transformers, AutoGPTQ, AutoAWQ) so any model format works. It exposes an OpenAI-compatible REST API so any code that calls OpenAI can switch to a local model by changing the base URL. The original project goal: become the AUTOMATIC1111 of text generation — the community standard for local LLM experimentation.</InfoBox>
          <p style={{ ...P(sub), marginBottom: 0 }}>The key difference from Ollama and LM Studio: oobabooga exposes every generation parameter — temperature, top-p, top-k, repetition penalty, mirostat, context length, rope scaling, and dozens more. It supports character cards (a standard format for defining AI personas), notebook mode for freeform text generation, multiple chat formats, and a rich extension system. For users who want maximum control over their inference — researchers, enthusiasts, developers building specialized applications — this level of control is the point. For users who just want to chat with a local model quickly, Ollama is faster to start.</p>
        </Block>
        ...
        <Block dark={dark} border={border} card={card}>
          ...
          <p style={{ ...P(sub), marginBottom: '1rem' }}>The loader determines which backend processes the model. Choosing the right loader is the most important configuration decision in oobabooga.</p>
        ...
        <Block dark={dark} border={border} card={card}>
          ...
          <p style={{ ...P(sub), marginBottom: '1rem' }}>Extensions are Python scripts in the extensions/ folder. Enable them from Session tab → Extensions. Each extension adds a new tab or modifies the UI. The extension system is what makes oobabooga far more capable than a simple chat interface.</p>
          <CardGrid dark={dark} border={border} color={color} items={[
        ...
        <Block dark={dark} border={border} card={card}>
          ...
          <p style={{ ...P(sub), marginBottom: '1rem' }}>The built-in API server makes oobabooga a drop-in replacement for the OpenAI API in any project. Launch with <code style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: '0.78rem', color: color, background: dark ? 'rgba(245,158,11,0.1)' : 'rgba(245,158,11,0.08)', padding: '0.1rem 0.35rem', borderRadius: 4 }}>python server.py --api</code> and the following endpoints are available at localhost:5000:</p>
          <Steps dark={dark} border={border} color={color} items={[
        ...
        <div style={{ padding: '1.5rem', borderRadius: 16, background: dark ? `${color}08` : `${color}06`, border: `2px solid ${color}28`, marginBottom: '1.25rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.875rem' }}>
            ...
            <span style={{ fontFamily: "'Orbitron', sans-serif", fontWeight: 900, fontSize: '0.78rem', letterSpacing: '0.08em', color }}>PROJECT — Build a Local Persona Chatbot with Voice Input</span>
          </div>
          <p style={{ fontSize: '0.9rem', color: txt, lineHeight: 1.8, margin: '0 0 1rem' }}>Use text-generation-webui to build a custom AI persona chatbot with speech-to-text input, then expose it via the OpenAI-compatible API and build a simple Python client that calls it. You will experience the full power-user workflow: install, load a GGUF model, enable the whisper_stt extension, define a character card, launch the API server, and call it from Python. All 100% local, 100% free.</p>
          <Steps dark={dark} border={border} color={color} items={[
          ...
          <div style={{ padding: '0.625rem 0.875rem', borderRadius: 8, background: dark ? 'rgba(245,158,11,0.08)' : 'rgba(245,158,11,0.06)', border: '1px solid rgba(245,158,11,0.2)', marginTop: '0.875rem' }}>
            <span style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: '0.6rem', color: color, letterSpacing: '0.08em' }}>TOTAL COST: ₹0 — oobabooga is free, models are free from HuggingFace, all inference is local</span>
          </div>
        </div>

        {/* Tip */}
        <div style={{ padding: '1.125rem 1.25rem', borderRadius: 14, background: dark ? 'rgba(245,158,11,0.07)' : 'rgba(245,158,11,0.07)', border: '1px solid rgba(245,158,11,0.2)', marginBottom: '1.25rem' }}>
          <div style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: '0.6rem', letterSpacing: '0.12em', color: color, marginBottom: '0.5rem' }}>⚡ PRO TIP</div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '2rem' }}>
          ...
          <button onClick={() => navigate('/ai-lab/local/litelllm')} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', padding: '0.65rem 1.1rem', borderRadius: 10, border: `1px solid ${border}`, background: 'none', cursor: 'pointer', color: sub, fontFamily: "'Rajdhani', sans-serif", fontSize: '0.875rem', fontWeight: 600 }}>
            LiteLLM <ChevronRight size={14} />
          </button>
        </div>
